    ...
    $(function(){
        ...
        var _table = $table.dataTable($.extend(true,{},CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
            ajax : function(data, callback, settings) {
                ...
                $.ajax({
                    type: "GET",
                    url: rootPath+"/car/displayCarList.action",
                    cache : false,  //禁用缓存
                    data: param,    //传入已封装的参数
                    dataType: "json",
...
//        });
    ...
    var userManage = {
        getQueryCondition : function(data) {
            ...
            if (data.order&&data.order.length&&data.order[0]) {
                switch (data.order[0].column) {
                    case 0:
                        param.orderColumn = "carid";//数据库列名称
                        break;
                    case 1:
                        param.orderColumn = "carnumber";//数据库列名称
                        break;
                    case 2:
                        param.orderColumn = "color";//数据库列名称
                        break;
                    case 3:
        ...
        editItemInit : function(item) {
            //编辑方法
            window.location.href="car/toUpdateCar.action?carid="+item.carId;
        },
        deleteItem : function(item) {
            //删除
            //alert("删除"+item.uid+"  "+item.userName);
            var result = confirm("是否确认删除该车辆")
            //确认后删除该用户，并查询第一页数据，
            if(result){
                window.location.href="car/doDeleteCar.action?carid="+item.carId;
            }

        },
        showItemDetail: function(item){
            //点击行
            alert("点击"+item.carId+"  "+item.carNumber);
        }
    };
